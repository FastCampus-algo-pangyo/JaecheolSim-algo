...
public class 리코쳇로봇 {
    ...
    min 보다 더 작은 이동횟수일 경우만 업데이트가 된다.
    */

    /* dp
    로봇이 어떤 위치에서 멈췄다, 그때의 위치를 [3,4]라고 할 때
    [3,4]에 오기 위해선 시작지점에서 12번의 이동이 필요할 수 도, 시작지점에서 20번의 이동이 필요할 수 도 있다.
    최소 움직임을 min[3][4]에 저장하게 되면, 시작지점에서 20번 움직여서 왔을 때, 이전에 12번만에 이동한 게 있으니
    ...
    */
    ...
    public void dfs(int[] curPos, int moveCount) {
        ...
        // 1) curPos가 1,3 인 지점에 move Count 12인 상태로
        // 2) curPos가 1,3 인 지점에 move Count 20인 상태로
        // 가지치기를 사용
